...
export default function ResultInclusive(props) {
  const classes = useStyles();
  const discretes = data[props.language];
      ...
      <Grid container  >
            ...
            <Grid item md={12} xs={12}>
              <Container className={classes.radarChartContainer}>
                <Paper sx={{ paddingY: "25px" }} >
                  <RadarChartLeadershipData language={props.language}></RadarChartLeadershipData>
                <Box sx = {{position : "relative", bottom : 50}}>{isLoading ? <CircularProgress /> : discretes.data1.map((discrete) => {
        ...
        <Grid item xs={12}>
          ...
          <Paper sx={{ marginBottom: "120px" }}>
            ...
            {isLoading ? <CircularProgress /> : discretes["data1"].map((discrete) => {
